...
export default function DevDotTo(props) {
	...
	const { className = '', darkMode = true } = props;
	useEffect(
		() => {
			fetch(`https://dev.to/api/articles?username=${props.username}`).then((res) => res.json()).then((data) => {
				setPost(data);
	...
	return (
		<div>
			{loading ? (
				...
				<div className={[ darkMode ? 'devcontainer' : 'devcontainer light', className ].join(' ')}>
					{posts.map((item) => (
									...
									<div className="devreact">
										<div className="devicons">
											<p>
												...
												<span className="postreact"> {item.positive_reactions_count}</span>
											</p>
											<p>
												<svg
													width="1.2em"
